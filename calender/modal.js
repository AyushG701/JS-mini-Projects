...
export function openViewAllModal(date, eventElements) {
  const modalContent = viewAllModalTemplate.content.cloneNode(true);
  modalContent.querySelector("[data-title]").textContent = format(
    date,
    "MMMM d, yyyy",
  );
  ...
  if (eventElements.length === 0) {
    ...
    eventElements.forEach((event) => modalContent.appendChild(event));
  }
